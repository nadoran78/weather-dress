...
- 지역별 단기 날씨 정보 조회(3일 : 오늘, 내일, 모레)
    - Open API를 통해 조회 및 캐시 저장(지역은 위경도로 또는 주소로 조회, default : 현재위치 위경도 조회)
...
- 지역별 중기 날씨 정보 조회(3일 후 ~ 7일 후)
    - Open API를 통해 조회 및 db 저장(지역은 시/도 레벨로 조회)
...
- 게시판
    - 게시판 글 작성
        ...
        - 회원이 활동한 지역 선택(시군구 레벨까지)
          - 지역 선택 시 해당 지역을 좌표로 변환하여 Open Api를 통해 최저 기온, 최고 기온 조회
    - 게시판 글 수정
        - 작성자만 수정 가능
    - 게시판 글 목록 조회
        - 페이징 처리
...
- [Geocoder API(위경도 주소 변환 API)](https://www.vworld.kr/dev/v4dv_geocoderguide2_s002.do)
